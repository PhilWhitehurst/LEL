...
<div class="page-header">
	<h2 class="entry-title"><?php echo apply_filters( 'woocommerce_my_account_my_downloads_title', __( 'Downloads', 'woocommerce' ) ); ?></h2>
</div>
...
<?php if ( $has_downloads ) : ?>
	...
	<table class="woocommerce-MyAccount-downloads shop_table shop_table_responsive table table-striped">
		<thead>
			<tr>
							...
							<?php
								...
								if ( $actions = apply_filters( 'woocommerce_account_download_actions', $actions, $download ) ) {
									...
									foreach ( $actions as $key => $action ) {
										echo '<a href="' . esc_url( $action['url'] ) . '" class="button woocommerce-Button btn btn-default ' . sanitize_html_class( $key ) . '">' . esc_html( $action['name'] ) . '</a>';
									}
									echo '</div>';
								}
							?>
